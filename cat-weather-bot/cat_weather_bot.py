...
class CatWeatherBot:
    ...
    def __get_random_image_showing_temperature(
        self, temperature: int | float
    ) -> str:
        path_images = f"{os.path.dirname(os.path.abspath(__file__))}/inc/img"
        print(temperature)
        temperature_representation = self.__define_temperature_representation(
            temperature
        )

        list_files_img_dir = os.listdir(
            f"{path_images}/{temperature_representation}/"
        )
        randomize_image = random.choice(list_files_img_dir)

        return f"{path_images}/{temperature_representation}/{randomize_image}"

    def __get_random_text_showing_temperature(
        self, temperature: int | float
    ) -> str:
    ...
    async def __location(
        ...
        temperature_info = self.open_weather_api.get_temperature(
        ...
        random_image = self.__get_random_image_showing_temperature(
            apparent_temperature
        )
        random_text = self.__get_random_text_showing_temperature(
            apparent_temperature
        )

        text_formated = random_text.format(
